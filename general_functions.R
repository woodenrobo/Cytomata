...
inject_fcs_gating <- function(input, filter_features, asinh_transform, cofac, sampling_rate = 1, silent = FALSE, event_cutoff = 0) {
    library(flowCore)
    library(progress)
    library(dplyr)
    cat('INJECTING DATA\n')
    ...
    for (f in input) {
        ...
        exprs <- as.data.frame(fcs@exprs)
        cat(nrow(exprs), "events in", rep(basename(f)), "\n")
        colnames(exprs) <- fcs@parameters@data$desc
    ...
    if (asinh_transform == TRUE) {
        scalable_features <- colnames(exprs_set)[grepl(paste(c(feature_markers,feature_markers_gating), collapse = "|"), colnames(exprs_set))]
        #transform the expression values
        exprs_set[, colnames(exprs_set) %in% scalable_features] <- asinh(exprs_set[, colnames(exprs_set) %in% scalable_features] / cofac)
        cat("Applied arcsinh transformation with the cofactor of", cofac, "\n")
    }
